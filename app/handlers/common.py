...
async def main_menu(callback: CallbackQuery):
    ...
    await log_event(user_id=callback.from_user.id, event_type="main_menu")
...
async def job_search_handler(callback: CallbackQuery):
    await callback.answer()
    await log_event(user_id=callback.from_user.id, event_type="job_search")
    await callback.message.edit_text(
...
async def region_selected_handler(callback: CallbackQuery, state: FSMContext):
    await callback.answer()
    await log_event(user_id=callback.from_user.id, event_type="region_selected")
...
async def about_company_handler(callback: CallbackQuery):
    await callback.answer()
    await log_event(user_id=callback.from_user.id, event_type="about_company")
...
async def about_company_handler(callback: CallbackQuery):
    await callback.answer()
    await log_event(user_id=callback.from_user.id, event_type="advantages")
...
async def about_company_handler(callback: CallbackQuery):
    await callback.answer()
    await log_event(user_id=callback.from_user.id, event_type="compnies_enterprises")
...
async def about_company_handler(callback: CallbackQuery):
    await callback.answer()
    await log_event(user_id=callback.from_user.id, event_type="main_directions")
...
async def contact_info_handler(callback: CallbackQuery):
    await callback.answer()
    await log_event(user_id=callback.from_user.id, event_type="contact_info")
...
async def social_links_handler(callback: CallbackQuery):
    await callback.answer()
    await log_event(user_id=callback.from_user.id, event_type="social_links")
